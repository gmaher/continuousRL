...
def genCrash(S0,r,sig,p_crash,dt,N,steps=10):
    ...
    Z = sig*np.sqrt(dt)*np.random.randn(N-1)
...
class Crashing:
    def __init__(self, interest, std_dev, p_crash,
        ...
        self.t = 0
        self.Nsteps = N
        self.total_length = N+lookback-1
    ...
    def reset(self):
        ...
        self.S,self.Z = genCrash(self.S0,self.interest,self.std_dev,
            self.p_crash,1,self.total_length,self.crash_steps)
    ...
    def step(self, action):
        ...
        reward = 0.0
        self.t += 1
        if self.t+self.lookback >= self.total_length-1:
            self.done = True

        self.observation_space.state = self.S[self.t:self.t+self.lookback]

        #calculate  return
        t = self.t+self.lookback-1
        r = action*(self.S[t]-self.S[t-1])/self.S[t-1]
